...
namespace DiversityCollection.UserControls
{
    ...
    /// </summary>
    public partial class UserControl_GIS : UserControl
    {
        #region Parameter
        ...
        /// </summary>
        public enum SelectedGisObject { Organism, Event, Series, AnyCoordinate, Distribution, DistributionIncludingOrganisms, None };
        ...
        /// </summary>
        public enum MapMode { /*Browse,*/ Webview, Edit, View };
        private MapMode _MapMode = MapMode.Webview;
        ...
        public UserControl_GIS(UserControls.iMainForm MainForm)
        {
            try
            {
                InitializeComponent();
                ...
                else
                {
                    ...
                    switch (_SelectedGisObject)
                    {
                        ...
                        case SelectedGisObject.Distribution:
                            ...
                            break;
                        case SelectedGisObject.None:
                            switch(this._MapMode)
                            {
                                case MapMode.Webview:
                                    this.setMapInBrowser("");
                                    break;
                            }
                            break;
        ...
        private WpfSamplingPlotPage.PointSymbol Symbol
        {
            get
            {
                try
                {
                    // Markus 2024-07-05 - Testing if null
                    if (this.toolStripDropDownButtonSymbol.Tag == null)
        ...
        { 
            get 
            {
                try
                {
                    // Markus - bugfix if image is missing
                    if (this.toolStripDropDownButtonSymbol.Image == null || this.toolStripDropDownButtonSymbol.Image.Tag == null)
                        return 32;
        ...
        private void setVisibilityOfControls()
        {
            ...
            switch (this._MapMode)
            {
                ...
                //    this.splitContainerWebbrowser.Panel2Collapsed = false;
                //    break;
                case MapMode.Webview:
                    this.splitContainerMaps.Panel1Collapsed = false;
                    this.splitContainerMaps.Panel2Collapsed = true;
                    this.splitContainerWebbrowser.Panel1Collapsed = true;// false;
                    this.splitContainerWebbrowser.Panel2Collapsed = false; // true;
                    break;
                default:
            ...
            if (this._MapMode == MapMode.Webview)
            {
                // Symbol
        ...
        {
            string URL = this.GetCoordinatesOfGeoObject(GeoObject, true, true, false);
            if (URL.Length > 2000)
            {
            ...
            if (URL.Length > 0)
            {
                try
                {
                    ...
                    if (!Uri.TryCreate(URL, UriKind.Absolute, out Uri URI))
                    {
                        ...
                        DiversityWorkbench.ExceptionHandling.WriteToErrorLogFile("Invalid URI Error in UserControl._GIS.setMapInBrowser method. URI: " + URL);
                        return;
                    }
                    
                    // Markus 2.10.2020: Versuch mit neuer Variante
                    switch (this._MapMode)
                    {
                        case MapMode.Webview:
                            this.splitContainerWebbrowser.Panel1Collapsed = true;// false;
                            this.splitContainerWebbrowser.Panel2Collapsed = false;// true;
                            this.userControlWebView.Url = URI;
                            //this.web Browser.Url = URI;
                            //this.webViewCompatible.Navigate(URL);
                            break;
                        //case MapMode.Browse:
                        //    this.splitContainerWebbrowser.Panel1Collapsed = true;
                        //    this.splitContainerWebbrowser.Panel2Collapsed = false;
        ...
        {
            this.toolStripDropDownButtonEditMode.Image = this.webviewToolStripMenuItem.Image;
            this.toolStripDropDownButtonEditMode.Text = this.webviewToolStripMenuItem.Text;
            this.toolStripDropDownButtonEditMode.ToolTipText = "Use web viewer to display the map";
            this._MapMode = MapMode.Webview;
            this.setVisibilityOfControls();
            if (this._SelectedGisObject == SelectedGisObject.Distribution || this._SelectedGisObject == SelectedGisObject.DistributionIncludingOrganisms)
                this.setMapInBrowser(this.DistributionObjectListBrowser);
            else if (this._SelectedGisObject == SelectedGisObject.Organism)
