...
namespace DiversityCollection.Tasks
{
    class CollectionTask : HierarchicalEntity
    {
        ...
        public static string sqlFieldCollectionTaskMetric = "CollectionTaskID, MetricDate, Aggregation, MetricValue";
        private Microsoft.Data.SqlClient.SqlDataAdapter _SqlDataAdapterCollectionTaskMetric;
        ...
        {
            this._sqlItemFieldList = " CollectionTaskID, CollectionTaskParentID, CollectionID, TaskID, DisplayOrder, DisplayText, CollectionSpecimenID, SpecimenPartID, TransactionID, ModuleUri, " +
                "TaskStart, TaskEnd, Result, URI, NumberValue, BoolValue, MetricDescription, MetricSource, MetricUnit, ResponsibleAgent, ResponsibleAgentURI, Description, Notes ";
        ...
        //          null, 
        //          ref BindingSource, 
        //          DiversityCollection.LookupTable.DtCollectionTaskHierarchy, 
        //          DiversityCollection.LookupTable.DtCollectionTaskHierarchy)
        //{
        //    this._sqlItemFieldList = " CollectionTaskID, CollectionTaskParentID, CollectionID, TaskID, DisplayOrder, DisplayText, CollectionSpecimenID, SpecimenPartID, TransactionID, ModuleUri, " +
        //        "TaskStart, TaskEnd, Result, URI, NumberValue, BoolValue, MetricDescription, MetricSource, MetricUnit, ResponsibleAgent, ResponsibleAgentURI, Description, Notes ";
        ...
        public override void fillDependentTables(int ID)
        {
            ...
            // Metric
            SQL = "SELECT " + sqlFieldCollectionTaskMetric +
                " FROM CollectionTaskMetric " +
                "WHERE CollectionTaskID = " + ID.ToString();
            Microsoft.Data.SqlClient.SqlDataAdapter adMetric = new Microsoft.Data.SqlClient.SqlDataAdapter(SQL, DiversityWorkbench.Settings.ConnectionString);
            this._DataSet.Tables["CollectionTaskMetric"].Clear();
            this.FormFunctions.initSqlAdapter(ref this._SqlDataAdapterCollectionTaskMetric, SQL, this._DataSet.Tables["CollectionTaskMetric"]);
        ...
        public override bool deleteDependentData(int ID)
        {
            try
            {
                ...
                SQL = "DELETE T FROM CollectionTask T " +
                    "WHERE T.CollectionTaskParentID = " + ID.ToString();
                DiversityWorkbench.Forms.FormFunctions.SqlExecuteNonQuery(SQL);

                // Metric
                SQL = "DELETE T FROM CollectionTaskMetric T " +
        ...
        public System.Data.DataTable DtTasks(int? ParentID = null)
        {
            ...
            try
            {
                ...
                {
                    string SqlTask = "SELECT T.[TaskID], T.HierarchyDisplayText AS DisplayText, T.HierarchyDisplayTextInvers AS DisplayTextInvers " +
                        "FROM dbo.TaskHierarchyAll() T WHERE T.TaskID = " + PresetTaskID.ToString();
                    DiversityWorkbench.Forms.FormFunctions.SqlFillTable(SqlTask, ref dtTasks);
                }
                else if (ParentID != null)
                {
                    string SqlTask = "SELECT T.[TaskID], T.HierarchyDisplayText AS DisplayText, T.HierarchyDisplayTextInvers AS DisplayTextInvers, T.Type AS [Type], T.ModuleType " +
                        "FROM [Task] P inner join dbo.TaskHierarchyAll() T on P.TaskID = T.TaskParentID INNER JOIN CollectionTask C ON C.TaskID = P.TaskID AND C.CollectionTaskID = " + ParentID.ToString() +
                        " WHERE T.SpecimenPartType <> '' OR T.DescriptionType <> '' OR T.ModuleType <> '' OR T.NumberType <> '' OR T.NotesType <> '' OR T.ResultType <> '' OR T.BoolType <> '' OR T.DateType <> '' OR T.UriType <> '' OR T.ModuleTitle <> '' OR T.MetricType <> '' OR T.ResponsibleType <> '' " +
                        " union " +
                        "SELECT B.[TaskID], B.[HierarchyDisplayText], B.[HierarchyDisplayTextInvers], B.Type AS [Type], B.ModuleType " +
                        "FROM [Task] P inner join Task T on P.TaskID = T.TaskParentID  INNER JOIN CollectionTask C ON C.TaskID = P.TaskID AND C.CollectionTaskID = " + ParentID.ToString() +
                        " inner join dbo.TaskHierarchyAll() B on T.Type = B.Type and B.DisplayText = T.DisplayText and B.TaskParentID is null Order By ModuleType, [Type] ";
                    DiversityWorkbench.Forms.FormFunctions.SqlFillTable(SqlTask, ref dtTasks);
                }
                else
                    dtTasks = DiversityCollection.LookupTable.DtTask;
        ...
        private DiversityCollection.Tasks.FormCollectionTask _formCollectionTask;
        public void setFormCollectionTask(DiversityCollection.Tasks.FormCollectionTask f)
        {
        ...
        private void writeXmlCollectionTask(ref System.Xml.XmlWriter W, int CollectionTaskID)
        {
            ...
            SQL = "SELECT T.DisplayText AS Task, T.ModuleTitle, CT.DisplayText, CT.ModuleUri, " + SqlDate +
                ...
                "C.CollectionName, C.CollectionAcronym, C.LocationPlan, P.CollectionName AS ParentCollection, P.CollectionAcronym AS ParentCollectionAcronym, " +
        ...
        private void writeXmlCollectionTaskChart(ref System.Xml.XmlWriter W, int CollectionTaskID)
        {
            bool CreateChart = this.CreateChart(CollectionTaskID);
        }

        private bool CreateChart(int CollectionTaskID)
        {
            string SQL = "SELECT COUNT(*) " +
                "FROM CollectionTask AS CT INNER JOIN " +
                "Collection AS C ON CT.CollectionID = C.CollectionID INNER JOIN " +
                "Task AS T ON CT.TaskID = T.TaskID AND C.Type = T.Type INNER JOIN " +
                "Collection AS R ON C.CollectionParentID = R.CollectionID " +
                "WHERE (R.Type = N'room') AND C.Type IN " +
                "('trap', 'sensor') AND CT.CollectionTaskID = " + CollectionTaskID.ToString();
            return false;
        }
        ...
        public static System.Collections.Generic.Dictionary<string, DiversityWorkbench.Import.Step> ImportStepsCollectionTask
        {
            get
            {
                ...
                DiversityWorkbench.Import.DataTable DT = ImportWizardDataTable("CollectionTask", "", "CollectionTask", DiversityWorkbench.Import.DataTable.Parallelity.unique, 1, "", "CollectionTaskID");
                ...
                DiversityWorkbench.Import.StepColumnGroup GCreator = ImportWizardStepColumnGroup("Creator and license",
                ...
                DiversityWorkbench.Import.DataTable DTImages = ImportWizardDataTable("CollectionTaskImage", "CollectionTask", "Image", DiversityWorkbench.Import.DataTable.Parallelity.parallel, 2, "", "");
                ...
                // Metric
                DiversityWorkbench.Import.DataTable DTMetric = ImportWizardDataTable("CollectionTaskMetric", "CollectionTask", "Metric", DiversityWorkbench.Import.DataTable.Parallelity.parallel, 3, "", "");
