...
declare @SourceDatabaseTrunk nvarchar(500)
begin try
set @SourceDatabaseTrunk = ( select dbo.SourceDatabaseTrunk())
--select 'SourceDatabaseTrunk: ' + @SourceDatabaseTrunk
end try
begin catch
	set @AllTestPassed = 'NO'
	set @Message = 'The function SourceDatabaseTrunk did not return a result. Please turn to your administrator to fix this before continuing with the update of the database'
	RAISERROR (@Message, 18, 1) 
end catch
...
--select 'SourceDatabaseTrunk: ' + @HighResolutionImagePath
end try
begin catch
	set @AllTestPassed = 'NO'
