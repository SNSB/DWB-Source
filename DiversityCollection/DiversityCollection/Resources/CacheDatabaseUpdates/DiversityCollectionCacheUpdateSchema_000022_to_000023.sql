...
if (select count(*) from INFORMATION_SCHEMA.COLUMNS C where C.TABLE_SCHEMA = '#project#' AND C.TABLE_NAME = 'CacheMetadata' and C.COLUMN_NAME = 'StableIdentifier') = 0
begin
ALTER TABLE [#project#].CacheMetadata ADD StableIdentifier [nvarchar](500) NULL
end
GO
...
--#####################################################################################################################
--######   procPublishMetadata - inclusion of StableIdentifier  #######################################################
--#####################################################################################################################
