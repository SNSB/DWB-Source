...
--#####################################################################################################################
--######   Transfer Annotation to CollectionSpecimenReference  ########################################################
--#####################################################################################################################

begin tran
begin try
-- Transfer to CollectionSpecimenReference
INSERT INTO [dbo].[CollectionSpecimenReference]
           ([CollectionSpecimenID],
		   [ReferenceTitle]
           ,[ReferenceURI], Notes,[ResponsibleName] ,[ResponsibleAgentURI]
           ,[LogCreatedWhen],[LogCreatedBy],[LogUpdatedWhen],[LogUpdatedBy])
SELECT a.[ReferencedID],
			case when a.[ReferenceDisplayText] is null then case when a.Annotation is null then '' else a.Annotation end else a.[ReferenceDisplayText] end,
			a.[ReferenceURI], a.Annotation,[SourceDisplayText] ,[SourceURI]
      ,a.[LogCreatedWhen],a.[LogCreatedBy],a.[LogUpdatedWhen],a.[LogUpdatedBy]
  FROM [dbo].[Annotation]
a where a.AnnotationType = 'Reference'
and a.ReferencedTable = 'CollectionSpecimen'

-- Removal of internal relations in Annotation
UPDATE A SET A.[ReferencedAnnotationID] = NULL
FROM Annotation A
WHERE A.[ReferencedAnnotationID] IN
(
	SELECT aa.AnnotationID
	FROM [dbo].[Annotation]
	aa where aa.AnnotationType = 'Reference'
	and aa.ReferencedTable = 'CollectionSpecimen'
)
and a.ReferencedTable = 'CollectionSpecimen'
...
-- Transfer to CollectionSpecimenReference
INSERT INTO [dbo].[CollectionSpecimenReference]
           ([CollectionSpecimenID],IdentificationUnitID,[ReferenceTitle],
		   [ReferenceURI], Notes,[ResponsibleName] ,[ResponsibleAgentURI]
           ,[LogCreatedWhen],[LogCreatedBy],[LogUpdatedWhen],[LogUpdatedBy])
SELECT U.CollectionSpecimenID,[ReferencedID],
			case when [ReferenceDisplayText] is null then case when a.Annotation is null then '' else a.Annotation end else [ReferenceDisplayText] end,
			[ReferenceURI], a.Annotation,[SourceDisplayText] ,[SourceURI]
      ,a.[LogCreatedWhen],a.[LogCreatedBy],a.[LogUpdatedWhen],a.[LogUpdatedBy]
  FROM [dbo].[Annotation] a, dbo.IdentificationUnit U 
  where a.AnnotationType = 'Reference'
and a.ReferencedTable = 'IdentificationUnit'
...
(
	...
	FROM [dbo].[Annotation]
	aa where aa.AnnotationType = 'Reference'
	and aa.ReferencedTable = 'IdentificationUnit'
)
and a.ReferencedTable = 'IdentificationUnit'
...
INSERT INTO [dbo].[CollectionSpecimenReference]
           ...
           ,[ReferenceURI], Notes,[ResponsibleName] ,[ResponsibleAgentURI]
           ,[LogCreatedWhen],[LogCreatedBy],[LogUpdatedWhen],[LogUpdatedBy])
SELECT P.CollectionSpecimenID,a.[ReferencedID],
			case when a.[ReferenceDisplayText] is null then case when a.Annotation is null then '' else a.Annotation end else a.[ReferenceDisplayText] end,
			[ReferenceURI], a.Annotation,[SourceDisplayText] ,[SourceURI]
      ,a.[LogCreatedWhen],a.[LogCreatedBy],a.[LogUpdatedWhen],a.[LogUpdatedBy]
  FROM [dbo].[Annotation] a, dbo.CollectionSpecimenPart P 
  where a.AnnotationType = 'Reference'
and a.ReferencedTable = 'CollectionSpecimenPart'
...
(
	...
	FROM [dbo].[Annotation]
	aa where aa.AnnotationType = 'Reference'
	and aa.ReferencedTable = 'CollectionSpecimenPart'
)
and a.ReferencedTable = 'CollectionSpecimenPart'
