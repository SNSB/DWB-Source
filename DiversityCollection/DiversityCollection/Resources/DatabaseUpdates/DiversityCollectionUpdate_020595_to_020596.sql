...
GRANT SELECT ON Identification_Core2 TO [User]
GO
...
GRANT SELECT ON IdentificationUnit_Core2 TO [User]
GO


--#####################################################################################################################
--######   GRANTs for ExternalIdentifier  #############################################################################
--#####################################################################################################################

GRANT SELECT ON dbo.[ExternalIdentifier] TO [User];
GRANT UPDATE ON dbo.[ExternalIdentifier] TO [Editor];
GRANT INSERT ON dbo.[ExternalIdentifier] TO [Editor];
GRANT DELETE ON dbo.[ExternalIdentifier] TO [Editor];

GRANT SELECT ON dbo.[ExternalIdentifier_log] TO [Editor];
GRANT INSERT ON dbo.[ExternalIdentifier_log] TO [Editor];

GRANT SELECT ON dbo.[ExternalIdentifierType] TO [User];
GRANT UPDATE ON dbo.[ExternalIdentifierType] TO [Administrator];
GRANT INSERT ON dbo.[ExternalIdentifierType] TO [Administrator];
GRANT DELETE ON dbo.[ExternalIdentifierType] TO [Administrator];

GRANT SELECT ON dbo.[ExternalIdentifierType_log] TO [Administrator];
GRANT INSERT ON dbo.[ExternalIdentifierType_log] TO [Administrator];


--#####################################################################################################################
--######   CollectionSpecimenProcessingMethodParameter - RowGUID   ####################################################
--#####################################################################################################################

ALTER TABLE CollectionSpecimenProcessingMethodParameter ADD [RowGUID] [uniqueidentifier] NOT NULL CONSTRAINT [DF_CollectionSpecimenProcessingMethodParameter_RowGUID]  DEFAULT (newsequentialid())
GO

ALTER TABLE CollectionSpecimenProcessingMethodParameter_log ADD [RowGUID] [uniqueidentifier]
GO
...
SELECT deleted.CollectionSpecimenID, deleted.SpecimenProcessingID, deleted.ProcessingID, deleted.MethodID, deleted.ParameterID, deleted.Value, deleted.MethodMarker, deleted.LogCreatedWhen, deleted.LogCreatedBy, deleted.LogUpdatedWhen, deleted.LogUpdatedBy, deleted.RowGUID,  @Version,  'D'
FROM DELETED
end
else
begin
INSERT INTO CollectionSpecimenProcessingMethodParameter_Log (CollectionSpecimenID, SpecimenProcessingID, ProcessingID, MethodID, ParameterID, Value, MethodMarker, LogCreatedWhen, LogCreatedBy, LogUpdatedWhen, LogUpdatedBy, RowGUID,  LogVersion, LogState) 
SELECT deleted.CollectionSpecimenID, deleted.SpecimenProcessingID, deleted.ProcessingID, deleted.MethodID, deleted.ParameterID, deleted.Value, deleted.MethodMarker, deleted.LogCreatedWhen, deleted.LogCreatedBy, deleted.LogUpdatedWhen, deleted.LogUpdatedBy, deleted.RowGUID, CollectionSpecimen.Version, 'D' 
...
-- CollectionRelation
IF (@IncludedTables LIKE '%|CollectionRelation|%')
BEGIN
...
GRANT EXEC ON dbo.procCopyCollectionSpecimen2 TO [Editor]
GO
