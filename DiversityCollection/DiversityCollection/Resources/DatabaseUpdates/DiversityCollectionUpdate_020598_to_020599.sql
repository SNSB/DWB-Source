...
ALTER TABLE [Transaction] ALTER COLUMN NumberOfUnits int NULL;
GO

ALTER TABLE [Transaction_log] ALTER COLUMN NumberOfUnits int NULL;
GO
...
--#####################################################################################################################
--######  TermUri in Identification  ##################################################################################
--#####################################################################################################################

IF (select count(*) from INFORMATION_SCHEMA.COLUMNS C where C.TABLE_NAME = 'Identification' and C.COLUMN_NAME = 'TermUri') = 0
begin
ALTER TABLE [dbo].[Identification] ADD [TermURI] varchar(500) NULL
ALTER TABLE [dbo].[Identification_log] ADD [TermURI] varchar(500) NULL
...
SELECT deleted.CollectionSpecimenID, deleted.IdentificationUnitID, deleted.IdentificationSequence, deleted.IdentificationDate, deleted.IdentificationDay, deleted.IdentificationMonth, deleted.IdentificationYear, deleted.IdentificationDateSupplement, deleted.IdentificationDateCategory, deleted.VernacularTerm, deleted.TermUri, deleted.TaxonomicName, deleted.NameURI, deleted.IdentificationCategory, deleted.IdentificationQualifier, deleted.TypeStatus, deleted.TypeNotes, deleted.ReferenceTitle, deleted.ReferenceURI, deleted.ReferenceDetails, deleted.Notes, deleted.ResponsibleName, deleted.ResponsibleAgentURI, deleted.RowGUID, deleted.LogCreatedWhen, deleted.LogCreatedBy, deleted.LogUpdatedWhen, deleted.LogUpdatedBy,  @Version,  'D'
FROM DELETED
end
else
begin
...
--#####################################################################################################################
--######  Rock and mineral as new TaxonomicGroup  #####################################################################
--#####################################################################################################################

IF (select count(*) from [CollTaxonomicGroup_Enum] T where T.code = 'rock') = 0
begin
...
IF (select count(*) from [CollTaxonomicGroup_Enum] T where T.code = 'mineral') = 0
begin
INSERT INTO [dbo].[CollTaxonomicGroup_Enum]
           ([Code]
           ,[Description]
